...
func process_message(buf []byte) ([]byte, bool) {
	var retmsg []byte
	var is_request bool
	if len(buf) == 0 {
		dummy := make([]byte, 0)
	...
	case 0x11: // start TDMA
		...
		retmsg, is_request = msg.WDC_ACK, true
	...
	case 0x15: // TDMA status
		...
		retmsg, is_request = msg.WDC_GET_TDMA_RES, true
	...
	case 0x17: // data request
		...
		retmsg, is_request = msg.WDC_MAC_DATA_CON, true

	default:
		fmt.Println("received wrong cmd")
		// send back WDC_ERROR
		msg.WDC_ERROR[2] = byte(msg.WRONG_CMD)
		retmsg, is_request = msg.WDC_ERROR, true
	}
	return retmsg, is_request
}
